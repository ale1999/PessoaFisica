...
public class PessoaFisica1 extends JFrame{
	...
	public PessoaFisica1(){
		...
		btnSalvar.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
				...
				pessoa.setEstado((String)cmbEst.getSelectedItem());
				pessoa.setTelefone(txtTel.getText());
				pessoa.setCelular(txtCel.getText());
				pessoa.setRg(txtRg.getText());
				pessoa.setCpf(txtCpf.getText());
				if(rdbsexo[0].isSelected())	pessoa.setSexo('f');
				else if(rdbsexo[1].isSelected()) pessoa.setSexo('m');
				
				Session session = HibernateUtil.getSessionFactory().openSession();
		    	
		    	PessoaHibernateDAO<PessoaFisica> pessoaDao = new PessoaHibernateDAO<PessoaFisica>(session);
	...
	public static void main(String args[]){
		PessoaFisica1 cadpf = new PessoaFisica1();
	}
}